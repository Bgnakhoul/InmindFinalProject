...
class HostAgent:
    ...
    def create_agent(self) -> Agent:
        return Agent(
            ...
            description="This Host agent is responsible orchastrate candidate matching process based on job description.",
    ...
    async def send_message(self, agent_name: str, task: str, tool_context: ToolContext):
        """Sends a candidate matching task for search_agent_RAG.""" 
...
def _get_initialized_host_agent_sync():
    ...
    async def _async_main():
        # Hardcoded URL for the search agent
        search_agent_RAG = [
            "http://localhost:10004",]

        print("initializing host agent")
        hosting_agent_instance = await HostAgent.create(
            remote_agent_addresses=search_agent_RAG
        )
